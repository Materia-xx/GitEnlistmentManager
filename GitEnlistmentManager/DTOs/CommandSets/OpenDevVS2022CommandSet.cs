...
namespace GitEnlistmentManager.DTOs.CommandSets
{
    public class OpenDevVS2022CommandSet : CommandSet
    {
        public OpenDevVS2022CommandSet()
        {
            ...
            // Though we still write a non-existing cmd if it doesn't exist. It won't work, but there will be an example in the default command sets folder to work with.
            ...
            Commands.Add(
                new RunProgramCommand()
                {
                    ...
                    Arguments = $@"-w gem nt --title ""{{RepoName}}"" --startingDirectory ""{{ReposFolder}}"" ""%comspec%"" /k \""\""{vsDevCmd}\""&&CD /d \""{{EnlistmentDirectory}}\""\""",
                    OpenNewWindow = true,
